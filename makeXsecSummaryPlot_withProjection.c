void makeXsecSummaryPlot_withProjection(){
    ...
    double Esys_comb = 0.185e-38;
    double Esys_comb_scale = Esys_comb/1e-38;
    ...
    // Calculated by scaling the present 2g0p, 2g1p, and semi-inclusive stat errors down by sqrt(1.78) (assuming 78% increase in analysis stats)
    ...
    // combo stat uncertainty: 0.06106/math.sqrt(1.78) = 0.04577
    double Estat_comb_projection_frac = 0.04577;
    ...
    double E_comb_projection_scale = sqrt(pow(Esys_comb_scale/xs_comb_scale,2)+pow(Estat_comb_projection_frac,2))*xs_comb_scale;
    ...
    double Estat_comb_projection_scale = Estat_comb_projection_frac*xs_comb_scale;

    std::cout<<"Estat_comb_projection_scale: "<<Estat_comb_projection_scale<<std::endl;
    std::cout<<"E_comb_projection_scale: "<<E_comb_projection_scale<<std::endl;
    ...
    Dat_comb->SetBinContent(45,xs_comb_scale);
    Dat_comb->SetBinError(45,E_comb_scale); //15
    ...
    Dat_comb->SetBinError(45,Estat_comb_scale);
    Dat_comb->Draw("E1p same");


    //************
    // Projected full data set combo data point/errors

    TH1D *Dat_comb_fullset = new TH1D("dat_full","dat_full",300,0.0,1.0);
    Dat_comb_fullset->SetBinContent(55,xs_comb_scale);
    Dat_comb_fullset->SetBinError(55,E_comb_projection_scale); //15
    Dat_comb_fullset->SetLineColor(kBlue);
    Dat_comb_fullset->SetMarkerColor(kBlue);
    Dat_comb_fullset->SetLineWidth(2);
    Dat_comb_fullset->SetMarkerStyle(20);
    Dat_comb_fullset->DrawCopy("E1p same");
    
    Dat_comb_fullset->SetBinError(55,Estat_comb_projection_scale);
    Dat_comb_fullset->DrawClone("E1p same");
    ...
    lb->AddEntry(Dat_comb, "Runs 1-3", "ep");
    lb->AddEntry(Dat_comb_fullset, "Runs 1-5 (projected)", "ep");
