...
parser.add_argument('response_dir', help = 'Path to response matrix directory', type = str, nargs = '?')
...
## If response_dir or out_dir is not provided, exit
if p.response_dir < 0:
  print "ERROR: Response directory argument not provided"
  parser.print_help()
  exit(1)
if p.out_dir < 0:
...
responseFileDir = p.response_dir
...
print "Copying response_matrices_exclusive.root to " + outFileDir
shutil.copy(responseFileDir + "/response_matrices_exclusive.root", outFileDir)
